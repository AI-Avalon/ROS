...
internal class ConfigProxy
{
    ...
    internal ConfigProxy(int chatPort)
    {
        ...
        {
            Trace.WriteLine("Windows 10より古いOSの場合：TLS 1.2を使用し、証明書の検証を無効にしてください。");
    ...
    private async Task ProxyAndRewriteResponseAsync(IHttpContext ctx)
    {
        ...
        Trace.WriteLine("URLへのクライアントプロキシ要求を受信: " + url);
        ...
        Trace.WriteLine("clientconfigサービスからステータスコード付きのレスポンスを受信した。: " + result.StatusCode);
